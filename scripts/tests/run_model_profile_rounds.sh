...
mkdir -p "${result_dir}/${PTA}_${TA}"
results_csv="${result_dir}/total_results.csv"
[[ -f "${results_csv}" ]] || \
    echo 'PTA,TA,round,time,kernel_column_11_total,profile_path,compile_path' > "${results_csv}"
for ((i = 1; i <= rounds; i++)); do
    rm -rf /tmp/torchinductor_root/*
    time="$(date +%Y%m%d_%H%M%S_%N)"
    run_dir="${result_dir}/${PTA}_${TA}/${time}"; mkdir -p "${run_dir}"; touch "${run_dir}/start"
    ...
    echo "${PTA},${TA},${i},${time},${total},${profile_path},${compile_path}" >> "${results_csv}"
done
