#!/usr/bin/env python3
"""Run the pre-decomposition embedding-backward fragment in eager and Inductor modes.

The input shapes are kept from the supplied ``eager_forward`` graph, while the
forward function calls ``aten.embedding_dense_backward`` directly. The script
has no Triton dependency and supports only CUDA and NPU devices.
"""
...
def eager_forward(where_4, ge_4, getitem_46, getitem_58, getitem_106, index_put_1):
    """The pre-decomposition eager graph, with device selected from its inputs."""
    ...
    grad_output = torch.ops.aten.where.self(unsqueeze_default_1, expand_61, full_default_92)
    return torch.ops.aten.embedding_dense_backward.default(
        grad_output, where_4, NUM_EMBEDDINGS, -1, False
    )
