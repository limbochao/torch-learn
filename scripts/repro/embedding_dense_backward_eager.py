#!/usr/bin/env python3
"""Run the extracted ``embedding_dense_backward`` eager fragment.

The supplied AOT graph contains a single eager operation: materializing a
``float32`` zero tensor with shape ``[10131227, 16]``.  This script keeps that
operation as an ATen call and compares eager execution with the Inductor path
on CUDA or NPU.  It intentionally does not import or call Triton directly.
"""
...
NUM_EMBEDDINGS = 10_131_227
EMBEDDING_DIM = 16


def eager_forward(device: torch.device):
    """Reconstructed eager graph from the supplied AOT fragment."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--check",
        action="store_true",
        help="compare eager and Inductor outputs (requires memory for two output tensors)",
    )
    parser.add_argument(
        "--profile-dir",
        type=Path,
        default=None,
        help="profile output root (default: prof_log/embedding_dense_backward_eager/<timestamp>)",
    )
...
def main() -> None:
    ...
    # Trigger compilation before profiling, matching the reference repro.
    compiled_forward(device)
    synchronize(device)

    if args.check:
        eager_result = eager_forward(device)
        inductor_result = compiled_forward(device)
        synchronize(device)
        torch.testing.assert_close(eager_result, inductor_result)
        print("check=passed")
        del eager_result, inductor_result

    profile_root = args.profile_dir or (
        Path("prof_log/embedding_dense_backward_eager")
        / datetime.now().strftime("%Y%m%d_%H%M%S_%f")
    )
    profile_root = profile_root.resolve()
    eager_us = device_time_us(eager_forward, device, args.warmup, args.repeat, profile_root / "eager")
    inductor_us = device_time_us(compiled_forward, device, args.warmup, args.repeat, profile_root / "inductor")
    print(f"device={device} device_id={args.device_id} shape=({NUM_EMBEDDINGS}, {EMBEDDING_DIM})")
