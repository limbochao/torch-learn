...
def triton_poi_fused_add_gelu_split_with_sizes_229(in_ptr0, in_ptr1, out_ptr0, ks0, z0_numel, y1_numel, x2_numel, Z0BLOCK, Y1BLOCK, Z0BLOCK_SUB : tl.constexpr, Y1BLOCK_SUB : tl.constexpr):
    ...
    for loop_z0 in range(loops_z0):
        ...
        for loop_y1 in range(loops_y1):
            ...
            x2_mask = x2 < x2_numel
            tmp0 = tl.load(in_ptr0 + (x2 + 1160*y1 + 1160*ks0*z0), x2_mask & y1_mask & z0_mask).to(tl.float32)
            tmp1 = tl.load(in_ptr1 + (x2 + 1160*z0), x2_mask & z0_mask).to(tl.float32)
            tmp2 = tmp0 + tmp1
            tmp3 = tmp2 * tmp2
            tmp4 = tmp3 * tmp2
            tmp5 = 0.044715
            ...
            tmp11 = tmp2 * tmp10
            tl.store(out_ptr0 + (x2 + 200*y1 + 200*ks0*z0), tmp11, x2_mask & y1_mask & z0_mask)
''', device_str='npu')
arg125_1 = 256
s0 = arg125_1
